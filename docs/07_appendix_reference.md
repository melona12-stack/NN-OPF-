...
그것을 전압과 묶는 함수입니다. **5단계(M5)** 에서 이 함수를 신경망으로 교체합니다.
...
> [!IMPORTANT] 이 숫자는 툴킷의 기준일 뿐입니다 — 우리 기준선은 따로 있습니다
> 툴킷이 말하는 선형 근사의 오차 수준은 전압 0.2%, 전력 1.5%입니다.
> 그런데 **우리가 직접 잰 선형 최소제곱은 그보다 훨씬 좋습니다** —
> 전력 오차가 case30 1.00%, case118 0.61%입니다
> ([06 문서](06_surrogate_training.md) §5.3).
>
> 그래서 넘어야 할 선은 1.5%가 아니라 **0.61~1.00%** 입니다.
> 외부 수치를 합격선으로 삼았으면 이미 넘은 줄 알았을 것입니다.

| 기법 | 전력 오차 (P/부하) | 비고 |
|---|---|---|
| **Bernstein 2017 LP** | < 1.5% | 툴킷 명시 — 외부 참고치 |
| **선형 최소제곱 (우리 구현)** | case30 **1.00%** · case118 **0.61%** | 실제 넘어야 할 선 |
| **MLP + 선형지름길 (우리)** | case30 **0.35%** ✅ · case118 1.14% ❌ | 초기 기록 |
| **MLP + `lstsq` 초기화 (우리)** | case118 무작위 **0.62%** · 미지 N-1 **1.10%** | 현재 최고. 선형에 1.02\~1.09배 짐 (06 §6.1) |
| 그래프 어텐션 GAT (M4) | case30 무작위 **1.10%** · 미지 N-1 **2.53%** | MLP + `lstsq`(1.46%)에 1.74배 짐 (06 §6.1) |
| DC 조류계산 | (측정 예정) | 가장 단순한 기준선 |
| Bolognani 2015 LP | (측정 예정) | 고정점 선형화 |
...
3. **기준선은 우리가 직접 측정한 선형 최소제곱이다** — case30 1.00%,
   case118 0.61% ([06](06_surrogate_training.md) §5.3). 툴킷의 0.2%/1.5%는
   외부 참고치일 뿐이다.
...
지금 위치는 **M4 막바지**입니다. 네 칸 중 두 칸을 이겼고, 남은 두 칸(case118)은
선형에 각각 1.8%·4.1% 차이까지 좁혔습니다. 4단계 기준선 결과는
